# Design — CRT Tools on NVMe (mergerFS Conflict)
...
├── .roms_base (NVMe) =NC  ← no new file creation
├── BATO-ALL/roms
├── BATO-PARROT/roms
└── BATO-LG/roms
...
/userdata/roms/crt/  →  currently on BATO-PARROT (wrong)
                        ...
                        required: must be on NVMe
```

### Why CRT Tools Must Be on NVMe
...
2. **Single-drive boot** — User may boot with only NVMe (no externals connected). CRT tools must be available.
...
[custom script or init.d]
  ...
  ↓
/userdata/roms/crt now points to NVMe (overlays mergerFS view)
  ↓
Mode switcher / EmulationStation read-write /userdata/roms/crt
  → all I/O goes to NVMe
```

### Migration Steps (One-Time)

1. Copy existing CRT content from BATO-PARROT to NVMe:
   ```bash
   mkdir -p /userdata/.roms_base/crt
